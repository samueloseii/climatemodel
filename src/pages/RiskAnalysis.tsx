        ...
        <div className="space-y-3">
          {tornadoData.map((item) => (
            <div key={item.variable} className="flex items-center gap-4">
              ...
              <div className="w-20 text-sm font-mono text-orange-400 shrink-0">{item.pct}% impact</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {results && (
        <>
          ...
          <div className="grid grid-cols-4 gap-4">
            {[
              { label: `VaR (${(confidence*100).toFixed(0)}%)`, value: fmt(var_val), desc: "Maximum expected loss", icon: Shield, color: "text-orange-400" },
              { label: `CVaR (${(confidence*100).toFixed(0)}%)`, value: fmt(cvar_val), desc: "Expected loss in worst tail", icon: AlertTriangle, color: "text-red-400" },
              { label: "Mean NPV", value: fmt(computeMean(npvValues)), desc: "Expected project value", icon: Shield, color: "text-emerald-400" },
              { label: "P(Loss)", value: `${(npvValues.filter(v => v < 0).length / npvValues.length * 100).toFixed(1)}%`, desc: "Probability of negative NPV", icon: AlertTriangle, color: npvValues.filter(v => v < 0).length / npvValues.length > 0.3 ? "text-red-400" : "text-emerald-400" },
            ].map(s => (
          ...
          <div className="glass-card p-4 flex items-center gap-4">
            ...
            {[0.90, 0.95, 0.99].map(c => (
              <button key={c} onClick={() => setConfidence(c)} className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${confidence === c ? "bg-orange-500/20 text-orange-400 border border-orange-500/30" : "text-slate-400 hover:text-slate-200"}`}>
                {(c*100).toFixed(0)}%
              </button>
            ))}
          </div>
          ...
          <div className="glass-card p-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/30">
                <p className="text-sm font-semibold text-orange-400 mb-2">Developer View</p>
              ...
              <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/30">
                <p className="text-sm font-semibold text-blue-400 mb-2">Lender View (CVaR)</p>
              ...
              <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/30">
                <p className="text-sm font-semibold text-purple-400 mb-2">Development Bank View</p>
                <p className="text-sm text-slate-300">Portfolio diversification benefit. P5 downside of <span className="text-orange-400 font-semibold">{fmt(computePercentile(npvValues, 0.05))}</span> is manageable within blended finance structure.</p>
              </div>
            </div>
          </div>
        </>
